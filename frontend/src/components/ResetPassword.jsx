...
const ResetPassword = () => {
  ...
  const emailFromForgot = location.state?.emailId || "";

  const [formData, setFormData] = useState({
    emailId: emailFromForgot,
    otp: "",
    newPassword: "",
    confirmPassword: "",
  });
    ...
    <div className="relative min-h-screen bg-linear-to-br from-indigo-50 via-purple-50 to-pink-100 flex justify-center items-center px-4 py-8">
      ...
      >
        ...
        <input
          ...
          onChange={handleChange}
          required
          maxLength={6} // optional, for 6-digit OTP
          className="w-full text-center tracking-widest text-lg font-semibold p-3 sm:p-4 rounded-2xl border-2 border-indigo-300 bg-indigo-50/70 focus:ring-4 focus:ring-indigo-200 focus:border-indigo-400 outline-none transition-all duration-300 shadow-md placeholder-gray-400"
        />
